...
bool CWalletTx::SubmitMemoryPoolAndRelay(std::string& err_string, bool relay, interfaces::Chain::Lock& locked_chain, std::string strCommand)
//
{
...
{
    return AvailableCoins(locked_chain, vCoins, fOnlySafe, coinControl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, fIncludeZeroValue, nCoinType, fUseInstantSend);
}
//
...
bool CWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, CValidationState& state, std::string strCommand)
//
{
    {
        auto locked_chain = chain().lock();
